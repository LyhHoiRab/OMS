...
import com.oms.core.trade.dao.mapper.LogisticsModifyRecordMapper;
...
@Repository
public class LogisticsModifyRecordDao{

    private Logger logger = LoggerFactory.getLogger(LogisticsModifyRecordDao.class);

    @Autowired
    private LogisticsModifyRecordMapper mapper;
    ...
    public Page<LogisticsModifyRecord> page(PageRequest pageRequest, String tradeId, Date minOperateTime, Date maxOperateTime){
        try{
            ...
            List<LogisticsModifyRecord> list  = mapper.find(criteria);
            Long                    total = mapper.count(criteria);

            return new Page<LogisticsModifyRecord>(list, total, pageRequest);
        }catch(Exception e){
